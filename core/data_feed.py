...
class MLSignalData(bt.feed.DataBase):
    ...
    def __init__(self, df=None, **kwargs):
        ...
        if df is not None:
            ...
            self.future_return = (
                df["future_return"].values if "future_return" in df.columns else np.zeros_like(self.close)
            )
...
class OptimizedDataFeed:
    ...
    def __del__(self):
        ...
        for mmap_obj in self.mmaps.values():
            mmap_obj.close()
        for fh in getattr(self, "_file_handles", []):
            fh.close()
    ...
    def _create_mmap(self, file_path: str) -> mmap.mmap:
        """Create memory-mapped file object. Keeps the file handle alive."""
        f = open(file_path, "rb")
...
class DataFeed:
    ...
    def get_historical_data(self, start_date: str | datetime, end_date: str | datetime) -> pd.DataFrame:
