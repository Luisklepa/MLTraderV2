...
def run_backtest(config_path: str, start_date: str, end_date: str, output_dir: str):
    ...
    for col, dtype in [("equity", float), ("position", float), ("drawdown", float), ("returns", float)]:
        ...
        try:
            ...
            for signal in signals:
                ...
                if signal["type"] == "buy":
                    ...
                    # Record trade entry
                    trades.append(
                        {"entry_time": df.index[i], "entry_price": entry_price, "size": position, "type": "long"}
                    )
                else:  # sell
                    position = -float(signal["size"])
                    entry_price = float(df["close"].iloc[i])
                    ...
                    # Record trade entry
                    trades.append(
                        {"entry_time": df.index[i], "entry_price": entry_price, "size": abs(position), "type": "short"}
                    )

            # Update results
            results.loc[df.index[i], "position"] = position
            results.loc[df.index[i], "equity"] = (
                results.loc[df.index[i - 1], "equity"]
                if results.loc[df.index[i], "equity"] == 100000
                else results.loc[df.index[i], "equity"]
            )
            results.loc[df.index[i], "returns"] = (
                results.loc[df.index[i], "equity"] / results.loc[df.index[i - 1], "equity"] - 1
                if results.loc[df.index[i - 1], "equity"] != 0
                else 0
            )
            peak = results.loc[: df.index[i], "equity"].max()
            results.loc[df.index[i], "drawdown"] = (
                (peak - results.loc[df.index[i], "equity"]) / peak if peak != 0 else 0
            )
    ...
    # Plot results
    plot_backtest_results(results, output_file=output_path / "backtest_results.html")
    ...
    sharpe_ratio = (
        np.sqrt(252) * results["returns"].mean() / results["returns"].std() if results["returns"].std() != 0 else 0
    )
...
def main():
    ...
    try:
        run_backtest(
            config_path=args.config, start_date=args.start_date, end_date=args.end_date, output_dir=args.output_dir
        )
    except Exception as e:
