...
from core.data_feed import MLSignalData, prepare_data
...
def plot_robustness_analysis(results_df: pd.DataFrame, metrics: dict[str, float], config: RobustnessConfig):
    ...
    # Returns distribution
    fig.add_trace(go.Histogram(x=results_df["pnl_total"], name="Returns", nbinsx=20), row=1, col=1)

    # Win rate by regime
    regime_cols = [col for col in results_df.columns if col.endswith("_win_rate")]
    regime_data = results_df[regime_cols].mean()
    fig.add_trace(go.Bar(x=regime_data.index, y=regime_data.values, name="Win Rate"), row=1, col=2)

    # Trade frequency
    fig.add_trace(go.Histogram(x=results_df["total_trades"], name="Trades", nbinsx=20), row=2, col=1)

    # Drawdown distribution
    fig.add_trace(go.Histogram(x=results_df["max_drawdown"], name="Drawdown", nbinsx=20), row=2, col=2)

    # Long vs Short performance
    fig.add_trace(
        go.Scatter(
            x=results_df["long_win_rate"], y=results_df["short_win_rate"], mode="markers", name="Direction Balance"
        ),
        row=3,
        col=1,
    )
    ...
    if "mc_results" in metrics:
        fig.add_trace(go.Histogram(x=metrics["mc_results"], name="Monte Carlo", nbinsx=30), row=3, col=2)
...
def run_robustness_analysis():
    ...
    # Combine all metrics
    all_metrics = {**robustness_metrics, **monte_carlo_metrics, **regime_metrics}
