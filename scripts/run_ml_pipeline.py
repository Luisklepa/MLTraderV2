...
def load_data(file_path: str) -> pd.DataFrame:
    ...
    dtype_dict = {"open": np.float64, "high": np.float64, "low": np.float64, "close": np.float64, "volume": np.float64}
...
def save_results(results: dict, output_dir: Path):
    ...
    with open(metrics_file, "w") as f:
        json.dump({"long_metrics": results["long_metrics"], "short_metrics": results["short_metrics"]}, f, indent=4)
...
def generate_plots(results: dict, output_dir: Path):
    ...
    for side in ["long", "short"]:
        plt.hist(preds[f"{side}_prob"], bins=50, alpha=0.5, label=f"{side.capitalize()} Probability")
