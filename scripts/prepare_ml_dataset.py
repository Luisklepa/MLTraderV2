...
def calculate_targets(df: pd.DataFrame, window: int, threshold: float) -> tuple[pd.Series, pd.Series]:
...
def prepare_ml_dataset(input_file: str, output_file: str, window: int = 10, threshold: float = 0.01) -> None:
    ...
    try:
        ...
        pipeline.generate_complete_dataset(
            file_path=input_file, output_path=output_file, thresholds=[threshold], windows=[window]
        )
...
def bayesian_optimize_dataset_separated(input_file, n_trials=50):
    ...
    print(
        f"Mejor combinación LONG: window={best_win_long}, threshold={best_thr_long:.4f}, ratio positivo={study_long.best_value:.4f}"
    )
    # --- Optimización SHORT ---
    ...
    print(
        f"Mejor combinación SHORT: window={best_win_short}, threshold={best_thr_short:.4f}, ratio positivo={study_short.best_value:.4f}"
    )
    return (best_win_long, best_thr_long, study_long.best_value), (
        best_win_short,
        best_thr_short,
        study_short.best_value,
    )
...
def main():
    ...
    parser.add_argument(
        "--output-combined", required=False, help="Output ML dataset file for combined LONG+SHORT signals"
    )
    parser.add_argument("--window-long", type=int, default=None, help="Target window size for LONG (si None, optimiza)")
    parser.add_argument(
        "--threshold-long", type=float, default=None, help="Target threshold for LONG (si None, optimiza)"
    )
    parser.add_argument(
        "--window-short", type=int, default=None, help="Target window size for SHORT (si None, optimiza)"
    )
    parser.add_argument(
        "--threshold-short", type=float, default=None, help="Target threshold for SHORT (si None, optimiza)"
    )
    ...
    try:
        ...
        if args.output_combined:
            ...
            if (
                args.window_long is None
                or args.threshold_long is None
                or args.window_short is None
                or args.threshold_short is None
            ):
                (best_win_long, best_thr_long, _), (best_win_short, best_thr_short, _) = (
                    bayesian_optimize_dataset_separated(args.input_file, n_trials=args.n_trials)
                )
            ...
            logger.info("[ADVANCED PIPELINE] Generando dataset COMBINADO...")
            pipeline = MLFeaturePipeline()
            ...
            logger.info("[ADVANCED PIPELINE] Dataset combinado generado correctamente.")
        else:
            ...
            if (
                ...
                (best_win_long, best_thr_long, best_ratio_long), (best_win_short, best_thr_short, best_ratio_short) = (
                    bayesian_optimize_dataset_separated(args.input_file, n_trials=args.n_trials)
                )
                window_long = best_win_long if args.window_long is None else args.window_long
                threshold_long = best_thr_long if args.threshold_long is None else args.threshold_long
                ...
                threshold_short = args.threshold_short
            if args.output_long:
                logger.info("[ADVANCED PIPELINE] Generando dataset LONG...")
                pipeline = MLFeaturePipeline()
                pipeline.generate_complete_dataset(
                    file_path=args.input_file,
            ...
            if args.output_short:
                logger.info("[ADVANCED PIPELINE] Generando dataset SHORT...")
                pipeline = MLFeaturePipeline()
                pipeline.generate_complete_dataset(
                    file_path=args.input_file,
            ...
            logger.info("[ADVANCED PIPELINE] Datasets generados correctamente.")
    except Exception as e:
