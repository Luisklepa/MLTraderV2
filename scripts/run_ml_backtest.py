...
from backtest.event_engine import BacktestEngine
...
def run_backtest(config_path: str, start_date: str, end_date: str, output_dir: str):
    ...
    reference_data = {asset: feed.get_historical_data(start, end) for asset, feed in reference_feeds.items()}

    # Initialize backtest engine
    engine = BacktestEngine(strategy=strategy, data=main_data, reference_data=reference_data, config=config)
    ...
    # Generate plots
    plot_backtest_results(results, output_path / f"backtest_plots_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html")
    ...
    print(
        f"Profit Factor: {(results['returns'][results['returns'] > 0].sum() / -results['returns'][results['returns'] < 0].sum()):.2f}"
    )
