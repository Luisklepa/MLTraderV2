...
def plot_walk_forward_results(results: list[dict[str, Any]], output_dir: Path):
    ...
    fig.add_trace(go.Scatter(y=train_drawdowns, name="Train Drawdown", mode="lines+markers"), row=2, col=1)
    fig.add_trace(go.Scatter(y=test_drawdowns, name="Test Drawdown", mode="lines+markers"), row=2, col=1)
    ...
    fig.add_trace(go.Box(y=ratios, name="Test/Train Ratio", boxpoints="all"), row=2, col=2)

    # Plot performance distribution
    fig.add_trace(go.Histogram(x=test_returns, name="Test Returns Dist"), row=3, col=1)
    ...
    fig.add_trace(go.Bar(x=param_names, y=param_values, name="Best Params"), row=3, col=2)

    # Update layout
    fig.update_layout(height=1200, showlegend=True, title_text="Walk-Forward Analysis Results")
...
def main():
    ...
    windows_above_min_trades = sum(
        1 for r in windows if r["test_metrics"]["total_trades"] >= config.MIN_TRADES_PER_WINDOW
    )
    windows_above_min_sharpe = sum(1 for r in windows if r["test_metrics"]["sharpe_ratio"] >= config.MIN_SHARPE_RATIO)
