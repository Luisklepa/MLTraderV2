...
def analyze_experiments_top10(
    csv_path="ml_experiments_top10.csv", output_path="experiment_results/insights_ml_experiments_top10.txt"
):
    ...
    if "n_features" in df.columns:
        lines.append(
            f"- Features usados: {df['n_features'].iloc[0]} (contextuales, técnicos, patrones, estructura de mercado, lags, etc)."
        )
    lines.append("\n| Dataset | Balanceo | Métrica | Score | Threshold | Retorno Total | Winrate | Trades |")
    lines.append("|---------|----------|---------|-------|-----------|---------------|---------|--------|")
    for _, row in df.iterrows():
        lines.append(
            f"| {os.path.basename(row['dataset'])} | {row['balance_strategy']} | {row['metric']} | {row['metric_score']:.2f} | {row['proba_thr']} | {row['total_return']:.2%} | {row['winrate']:.2%} | {int(row['n_trades']) if not pd.isnull(row['n_trades']) else '-'} |"
        )
    lines.append("\n## Insights automáticos\n")
    # Insights clave
    best = df.iloc[0]
    lines.append(
        f"- Mejor experimento: {os.path.basename(best['dataset'])} | Retorno: {best['total_return']:.2%} | Winrate: {best['winrate']:.2%} | Trades: {int(best['n_trades'])}"
    )
    lines.append(
    ...
    lines.append("- El número de trades es suficiente para robustez estadística (80-300 trades por experimento top).\n")
