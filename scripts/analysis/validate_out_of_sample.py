...
if len(selected_features) < len(importances):
    ...
    plt.plot(
        range(len(equity_curve_train), len(equity_curve_train) + len(equity_curve_out)),
        equity_curve_out,
        label="Out-of-sample (Test, filtrados)",
    )
    ...
    print(
        f"IN-SAMPLE: Total trades: {sum(y_pred_train)}, Win rate: {np.mean((y_pred_train == 1) & (y_train == 1)):.2%}, Total return: {sum(returns_train):.2%}, Return per trade: {np.mean([r for r in returns_train if r != 0]):.3%}, Final equity: {equity_curve_train[-1]:.3f}"
    )
    print(
        f"OUT-OF-SAMPLE: Total trades: {sum(y_pred_out)}, Win rate: {np.mean((y_pred_out == 1) & (y_out == 1)):.2%}, Total return: {sum(returns_out):.2%}, Return per trade: {np.mean([r for r in returns_out if r != 0]):.3%}, Final equity: {equity_curve_out[-1]:.3f}"
    )
