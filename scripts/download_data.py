...
def get_binance_klines(symbol: str, interval: str, start_time: int, end_time: int, limit: int = 1000) -> list:
...
def download_historical_data(symbol: str, interval: str, days: int, output_file: str) -> None:
    ...
    logger.info(
        f"[INFO] Rango solicitado (alineado): {datetime.utcfromtimestamp(start_time / 1000)} a {datetime.utcfromtimestamp(end_time / 1000)}"
    )
    ...
    while current_start < end_time:
        try:
            klines = get_binance_klines(
                symbol=symbol, interval=interval, start_time=current_start, end_time=end_time, limit=1000
            )
            chunk_count += 1
            if not klines:
            ...
            logger.info(
                f"[INFO] Chunk {chunk_count}: descargadas {len(klines)} velas. Total acumulado: {len(all_klines)}"
            )
            if len(klines) < 1000:
                logger.info(
                    f"[ADVERTENCIA] Chunk {chunk_count} devolvió menos de 1000 velas. Puede que no haya más datos disponibles."
                )
    ...
    if len(df) < expected_candles * 0.8:
        logger.warning(
            f"[ADVERTENCIA] Se descargaron muchas menos velas ({len(df)}) de las esperadas ({expected_candles}). Verifica el símbolo, el intervalo o el histórico disponible en Binance."
        )
...
def main():
    ...
    try:
        download_historical_data(
            symbol=args.symbol, interval=args.interval, days=args.days, output_file=args.output_file
        )
    except Exception as e:
