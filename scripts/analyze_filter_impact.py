...
def analyze_signals_with_filters(df, signal_type="all"):
    ...
    if len(remaining) > 0:
        ...
        print(
            f"Win rate esperado: {(remaining['future_return'] * np.where(signal_type == 'short', -1, 1) > 0).mean() * 100:.1f}%"
        )
    ...
    if len(remaining) > 0:
        ...
        for i, (lower, upper) in enumerate(zip([0] + thresholds[:-1], thresholds)):
            ...
            if mask.any():
                ...
                win_rate = (
                    signals_in_group["future_return"] * np.where(signal_type == "short", -1, 1) > 0
                ).mean() * 100
