...
class MarketRegime:
    ...
    def identify_regime(self, df: pd.DataFrame) -> pd.Series:
        ...
        vol_percentile = volatility.rolling(self.lookback_period).apply(lambda x: pd.Series(x).rank(pct=True).iloc[-1])
...
class MLSignalFilter:
    ...
    def apply_filters(self, df: pd.DataFrame, side: str, higher_tf_data: pd.DataFrame | None = None) -> pd.Series:
        """
    ...
    def _apply_volatility_filter(self, df: pd.DataFrame, side: str) -> pd.Series:
        ...
        atr_percentile = atr.rolling(20).apply(lambda x: pd.Series(x).rank(pct=True).iloc[-1])
    ...
    def _apply_volume_filter(self, df: pd.DataFrame, side: str) -> pd.Series:
        ...
        # Combine filters
        return (
            (volume_ratio >= config["volume_ratio_min"]) & (volume_trend > -0.2)  # Avoid declining volume
        )
    ...
    def _apply_higher_tf_filter(self, df: pd.DataFrame, higher_tf_data: pd.DataFrame, side: str) -> pd.Series:
        """
        Apply confirmation filters using higher timeframe data.
        """
        self.config[side]["filters"].get("higher_tf_confirm", {})
