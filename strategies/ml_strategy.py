...
class MLStrategy(bt.Strategy):
    ...
    def update_stops(self):
        ...
        for _data_name, pos in list(self.active_positions.items()):
...
class EnhancedMLStrategy:
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ATR
        df.loc[:, "atr"] = talib.ATR(df["high"], df["low"], df["close"], timeperiod=14)
    ...
    def generate_signals(self, df: pd.DataFrame) -> list[dict]:
        ...
        if (
            current["close"] > current["sma_20"]  # Price above SMA20
            and current["rsi"] > 50  # RSI above 50
            and current["macd"] > current["macd_signal"]  # MACD crossover
            and prev["macd"] <= prev["macd_signal"]
            and self.position <= 0  # Not already long
        ):
            # Calculate position size
            ...
            signals.append({"type": "buy", "size": size, "stop_loss": stop_loss, "reason": "long_entry"})

        # Check for short entry
        elif (
            current["close"] < current["sma_20"]  # Price below SMA20
            and current["rsi"] < 50  # RSI below 50
            and current["macd"] < current["macd_signal"]  # MACD crossover
            and prev["macd"] >= prev["macd_signal"]
            and self.position >= 0  # Not already short
        ):
            # Calculate position size
            size = self.calculate_position_size(current)
            ...
            signals.append({"type": "sell", "size": size, "stop_loss": stop_loss, "reason": "short_entry"})

        # Check for exit
        elif self.position != 0:
            if (
                (self.position > 0 and current["close"] < current["sma_20"])  # Long exit
                or (self.position < 0 and current["close"] > current["sma_20"])  # Short exit
            ):
                signals.append(
                    {"type": "sell" if self.position > 0 else "buy", "size": abs(self.position), "reason": "exit"}
                )
