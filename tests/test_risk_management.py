...
class TestPositionSizing:
    def test_higher_confidence_larger_position(self):
        ...
        size_high = rm.calculate_position_size(
            prediction_probability=0.9, volatility=0.02,
            correlation_risk=0.0, current_exposure=0.0, account_size=100_000,
        )
        size_low = rm.calculate_position_size(
            prediction_probability=0.5, volatility=0.02,
            correlation_risk=0.0, current_exposure=0.0, account_size=100_000,
        )
    ...
    def test_higher_volatility_smaller_position(self):
        ...
        size_low_vol = rm.calculate_position_size(
            prediction_probability=0.8, volatility=0.01,
            correlation_risk=0.0, current_exposure=0.0, account_size=100_000,
        )
        size_high_vol = rm.calculate_position_size(
            prediction_probability=0.8, volatility=0.10,
            correlation_risk=0.0, current_exposure=0.0, account_size=100_000,
        )
    ...
    def test_position_respects_max_size(self):
        ...
        size = rm.calculate_position_size(
            prediction_probability=1.0, volatility=0.001,
            correlation_risk=0.0, current_exposure=0.0, account_size=100_000,
        )
    ...
    def test_full_exposure_blocks_new_position(self):
        ...
        size = rm.calculate_position_size(
            prediction_probability=0.9, volatility=0.02,
            correlation_risk=0.0, current_exposure=1.0, account_size=100_000,
        )
        assert size == 0.0
...
class TestAdaptiveStops:
    def test_long_stop_below_entry(self):
        ...
        sl, tp = rm.calculate_adaptive_stops(
            ...
            trend_strength=0.5, position_type="long",
        )
        assert sl < 50000.0
        assert tp > 50000.0
    ...
    def test_short_stop_above_entry(self):
        ...
        sl, tp = rm.calculate_adaptive_stops(
            entry_price=50000.0, atr=500.0,
            volatility=pd.Series([0.02] * 20),
            trend_strength=0.5, position_type="short",
        )
        assert sl > 50000.0
        assert tp < 50000.0
    ...
    def test_min_stop_distance_enforced(self):
        ...
        sl, _ = rm.calculate_adaptive_stops(
            entry_price=50000.0, atr=1.0,
            volatility=pd.Series([0.001] * 20),
            trend_strength=0.0, position_type="long",
        )
...
class TestPortfolioConstraints:
    def test_max_positions_blocks_new(self):
        ...
        correlations = pd.DataFrame(
            [[1.0, 0.5], [0.5, 1.0]], index=["BTC", "ETH"], columns=["BTC", "ETH"]
        )
