...
class StrategyVisualizer:
    ...
    def plot_monthly_returns_simple(self, output_file: Path):
        ...
        # Update layout
        fig.update_layout(
            title="Monthly Returns", xaxis_title="Month", yaxis_title="Return (%)", template="plotly_white"
        )
    ...
    def plot_equity_curve(self, use_plotly: bool = True):
        ...
        if use_plotly:
            ...
            # Equity curve
            fig.add_trace(
                go.Scatter(x=equity.index, y=equity.values, name="Equity", line=dict(color="blue")), row=1, col=1
            )

            # Drawdown
            fig.add_trace(
                go.Scatter(
                    x=drawdown.index, y=drawdown.values, name="Drawdown", fill="tozeroy", line=dict(color="red")
                ),
                row=2,
                col=1,
            )

            fig.update_layout(title="Strategy Equity Curve and Drawdown", showlegend=True, height=800)
    ...
    def plot_trade_distribution(self, use_plotly: bool = True):
        ...
        if use_plotly:
            fig = make_subplots(rows=1, cols=2, subplot_titles=("Long Trades", "Short Trades"))

            # Long trades histogram
            fig.add_trace(go.Histogram(x=long_pnls, name="Long Trades", nbinsx=20, marker_color="green"), row=1, col=1)

            # Short trades histogram
            fig.add_trace(go.Histogram(x=short_pnls, name="Short Trades", nbinsx=20, marker_color="red"), row=1, col=2)

            fig.update_layout(title="Distribution of Trade Returns", showlegend=True, height=400)
    ...
    def plot_monthly_returns(self, use_plotly: bool = True):
        ...
        pivot = monthly_returns.pivot_table(values="returns", index="year", columns="month", aggfunc="sum")
    ...
    def _calculate_equity_curve(self) -> pd.Series:
        ...
        for trade in self.trades:
            self.df.index.get_loc(trade["entry_time"])
...
def plot_backtest_results(results: pd.DataFrame, output_file: Path) -> None:
    ...
    # Plot equity curve
    fig.add_trace(
        go.Scatter(x=results.index, y=results["equity"], name="Equity", line=dict(color="blue")), row=1, col=1
    )

    # Plot drawdown
    fig.add_trace(
        go.Scatter(
            x=results.index, y=results["drawdown"] * 100, name="Drawdown", fill="tozeroy", line=dict(color="red")
        ),
        row=1,
        col=2,
    )
    ...
    fig.add_trace(
        go.Histogram(x=daily_returns * 100, name="Daily Returns", nbinsx=50, marker_color="green"), row=2, col=1
    )

    # Plot position size
    fig.add_trace(
        go.Scatter(x=results.index, y=results["position"], name="Position", line=dict(color="purple")), row=2, col=2
    )
    ...
    fig.add_trace(
        go.Scatter(x=results.index, y=rolling_sharpe, name="Rolling Sharpe", line=dict(color="orange")), row=3, col=1
    )
    ...
    fig.add_trace(
        go.Scatter(x=results.index, y=rolling_wins, name="Rolling Win Rate", line=dict(color="blue")), row=3, col=2
    )

    # Update layout
    fig.update_layout(title="Backtest Results", showlegend=True, height=1200, width=1600, template="plotly_white")
...
def plot_trade_distribution(trades: list[dict], output_file: str) -> None:
    ...
    try:
        ...
        fig = make_subplots(
            rows=2,
            cols=2,
            subplot_titles=("PnL Distribution", "Trade Duration Distribution", "Cumulative PnL", "Win Rate by Hour"),
        )

        # Plot PnL distribution
        fig.add_trace(go.Histogram(x=df["pnl"], name="PnL Distribution", nbinsx=50, marker_color="blue"), row=1, col=1)

        # Plot trade duration distribution
        fig.add_trace(
            go.Histogram(x=df["duration"], name="Duration Distribution", nbinsx=50, marker_color="green"), row=1, col=2
        )

        # Plot cumulative PnL
        df["cumulative_pnl"] = df["pnl"].cumsum()
        fig.add_trace(
            go.Scatter(x=df.index, y=df["cumulative_pnl"], name="Cumulative PnL", line=dict(color="purple", width=1)),
            row=2,
            col=1,
        )
        ...
        fig.add_trace(
            go.Bar(x=win_rate_by_hour.index, y=win_rate_by_hour.values, name="Win Rate by Hour", marker_color="orange"),
            row=2,
            col=2,
        )

        # Update layout
        fig.update_layout(title="Trade Distribution Analysis", height=800, showlegend=True, template="plotly_white")
...
def plot_monthly_returns(results: pd.DataFrame, output_file: str) -> None:
    ...
    try:
        ...
        returns_matrix = monthly_returns.to_frame().pivot_table(
            index=monthly_returns.index.month, columns=monthly_returns.index.year, values="returns"
        )
...
def plot_risk_metrics(results: pd.DataFrame, output_file: str) -> None:
    ...
    try:
        ...
        fig.add_trace(
            go.Scatter(x=results.index, y=rolling_returns, name="Annual Returns", line=dict(color="blue", width=1)),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(x=results.index, y=rolling_vol, name="Volatility", line=dict(color="red", width=1)), row=1, col=2
        )

        # Plot rolling Sharpe ratio
        fig.add_trace(
            go.Scatter(x=results.index, y=rolling_sharpe, name="Sharpe Ratio", line=dict(color="green", width=1)),
            row=2,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(x=results.index, y=rolling_drawdown, name="Max Drawdown", line=dict(color="purple", width=1)),
            row=2,
            col=2,
        )

        # Update layout
        fig.update_layout(title="Rolling Risk Metrics", height=800, showlegend=True, template="plotly_white")
