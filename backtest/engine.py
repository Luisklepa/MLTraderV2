...
class BacktestEngine:
    ...
    def run_backtest(
        self, strategy_class: type[bt.Strategy], data: pd.DataFrame, strategy_params: dict = None
    ) -> BacktestResult:
    ...
    def optimize_strategy(
        self, strategy_class: type[bt.Strategy], data: pd.DataFrame, param_grid: list[dict]
    ) -> list[BacktestResult]:
    ...
    def compare_strategies(
        self, strategies: dict[str, type[bt.Strategy]], data: pd.DataFrame
    ...
    def plot_friendly_plotly(self, df, strat):
        ...
        fig = make_subplots(
            ...
            subplot_titles=("BTCUSDT Candlestick", "Equity Curve", "Volume")
            if volume is not None
            else ("BTCUSDT Candlestick", "Equity Curve"),
        )
        # --- Candlestick ---
        fig.add_trace(
            go.Candlestick(
        ...
        if volume is not None:
            ...
            fig.add_trace(
                go.Bar(x=df["open_time"], y=volume, marker_color=colors, name="Volume", showlegend=False, yaxis="y3"),
                row=3,
                col=1,
            )
        # --- Layout ---
    ...
    def calculate_robust_score(self, backtest_result, profit_factor):
        ...
        if (
            backtest_result.total_trades < 10
            or backtest_result.max_drawdown > 25
            or backtest_result.sharpe_ratio < 0.5
