...
def calculate_robustness_metrics(results: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    # Consistency metrics
    metrics.update({
        'return_cv': float(metrics['return_std'] / abs(metrics['return_mean']) if abs(metrics['return_mean']) > 1e-12 else 10.0),
        'win_rate_cv': float(metrics['win_rate_std'] / metrics['win_rate_mean'] if metrics['win_rate_mean'] > 1e-12 else 10.0),
        'trades_cv': float(metrics['trades_std'] / metrics['trades_mean'] if metrics['trades_mean'] > 1e-12 else 10.0)
    })
    ...
    metrics.update({
        ...
        'drawdown_cv': float(np.std(drawdowns) / abs(np.mean(drawdowns)) if abs(np.mean(drawdowns)) > 1e-12 else 10.0)
    })
    ...
    metrics.update({
        'long_short_balance': float(np.mean(long_rates) / np.mean(short_rates) if np.mean(short_rates) > 1e-12 else 10.0),
...
def calculate_regime_metrics(results: List[Dict[str, Any]], market_regimes: pd.Series) -> Dict[str, float]:
    ...
    metrics['regime_consistency'] = float(np.std(regime_means) / abs(np.mean(regime_means)) if abs(np.mean(regime_means)) > 1e-12 else 10.0)
