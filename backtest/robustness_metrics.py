...
def calculate_robustness_metrics(results: list[dict[str, Any]]) -> dict[str, float]:
    ...
    metrics.update(
        {
            "return_cv": float(
                metrics["return_std"] / abs(metrics["return_mean"]) if abs(metrics["return_mean"]) > 1e-12 else 10.0
            ),
            "win_rate_cv": float(
                metrics["win_rate_std"] / metrics["win_rate_mean"] if metrics["win_rate_mean"] > 1e-12 else 10.0
            ),
            "trades_cv": float(
                metrics["trades_std"] / metrics["trades_mean"] if metrics["trades_mean"] > 1e-12 else 10.0
            ),
        }
    )
    ...
    if len(returns) > 1:
        ...
        metrics.update({"returns_t_stat": float(t_stat), "returns_p_value": float(p_value)})
    ...
    metrics.update(
        {
            ...
            "drawdown_cv": float(
                np.std(drawdowns) / abs(np.mean(drawdowns)) if abs(np.mean(drawdowns)) > 1e-12 else 10.0
            ),
        }
    )
    ...
    metrics.update(
        {
            "long_short_balance": float(
                np.mean(long_rates) / np.mean(short_rates) if np.mean(short_rates) > 1e-12 else 10.0
            ),
...
def calculate_monte_carlo_metrics(trades: list[dict[str, Any]], n_simulations: int = 1000) -> dict[str, float]:
    ...
    metrics = {
        ...
        "mc_sharpe": float(
            np.mean(simulation_results) / np.std(simulation_results) if np.std(simulation_results) > 0 else 0
        ),
...
def calculate_regime_metrics(results: list[dict[str, Any]], market_regimes: pd.Series) -> dict[str, float]:
    ...
    for result in results:
        try:
            ...
            regime_slice = market_regimes[start_date:end_date]
            if regime_slice.empty:
                continue
            regime = regime_slice.mode().iloc[0]
        ...
        metrics.update(
            {
                ...
                f"{regime}_sharpe": float(
                    np.mean(regime_returns) / np.std(regime_returns) if np.std(regime_returns) > 0 else 0
                ),
    ...
    metrics["regime_consistency"] = float(
        np.std(regime_means) / abs(np.mean(regime_means)) if abs(np.mean(regime_means)) > 1e-12 else 10.0
    )
...
def calculate_robustness_score(metrics: dict[str, float], config: Any) -> float:
    ...
    # Return consistency (weight: 0.3) — clamp CV to finite values
    return_score = max(0, 1 - min(metrics["return_cv"], 10.0) / 2)
    ...
    direction_score = max(0, 1 - abs(1 - min(metrics["long_short_balance"], 10.0)))
