...
class WalkForwardAnalyzer:
                ...
                self.windows.append(
                    {
                        ...
                        "train_end": self.df.index[train_end_idx],
                        "test_start": self.df.index[test_start_idx],
                        "test_end": self.df.index[test_end_idx],
                    }
                )
    ...
    def run_analysis(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        for i, window in enumerate(self.windows, 1):
            logger.info("Window %d/%d", i, len(self.windows))
            ...
            if train_data.empty or test_data.empty:
                logger.warning(
                    "Skipping window %d: train_data.empty=%s, test_data.empty=%s", i, train_data.empty, test_data.empty
                )
                continue

            # Print periods
            logger.info("Train period: %s to %s", train_data.index[0], train_data.index[-1])
            logger.info("Test period: %s to %s", test_data.index[0], test_data.index[-1])
            ...
            train_results = run_backtest(data=train_data, initial_capital=100000, commission=0.001, **params)

            # Run backtest on test data
            test_results = run_backtest(data=test_data, initial_capital=100000, commission=0.001, **params)
    ...
    def optimize_parameters(self, param_grid: dict[str, list[Any]]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
        ...
        logger.info("Optimizing parameters over %d combinations...", len(param_combinations))
        ...
        for i, params in enumerate(param_combinations, 1):
            logger.info("Testing combination %d/%d: %s", i, len(param_combinations), params)

            # Run walk-forward analysis
            logger.info("Running walk-forward analysis with %d windows...", len(self.windows))
            ...
            # Store results
            result = {"params": params, "avg_test_sharpe": avg_test_sharpe, "window_results": wf_results}
    ...
    def plot_results(self, results: list[dict[str, Any]], output_dir: Path = None):
        ...
        # Add traces
        fig.add_trace(go.Scatter(y=train_sharpes, name="Train Sharpe", mode="lines+markers"), row=1, col=1)
        fig.add_trace(go.Scatter(y=test_sharpes, name="Test Sharpe", mode="lines+markers"), row=1, col=1)

        fig.add_trace(go.Scatter(y=train_returns, name="Train Returns", mode="lines+markers"), row=1, col=2)
        fig.add_trace(go.Scatter(y=test_returns, name="Test Returns", mode="lines+markers"), row=1, col=2)

        fig.add_trace(go.Scatter(y=train_win_rates, name="Train Win Rate", mode="lines+markers"), row=2, col=1)
        fig.add_trace(go.Scatter(y=test_win_rates, name="Test Win Rate", mode="lines+markers"), row=2, col=1)

        fig.add_trace(go.Scatter(y=train_profit_factors, name="Train PF", mode="lines+markers"), row=2, col=2)
        fig.add_trace(go.Scatter(y=test_profit_factors, name="Test PF", mode="lines+markers"), row=2, col=2)

        # Update layout
        fig.update_layout(height=800, title_text="Walk-Forward Analysis Results", showlegend=True)
